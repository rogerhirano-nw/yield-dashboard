...
| **beehiiv newsletters** | `beehiiv_client.py` (`BeehiivClient`) | `beehiiv_publications`, `beehiiv_posts` | Newsletter audience + per-send performance from the beehiiv v2 REST API. **Not an ad feed** — it reports the *audience behind* newsletter inventory, not revenue. `beehiiv_publications` is a **daily snapshot history** (the API exposes only current subscriber counts, so the trend has to be accumulated — same shape as `opensincera_ecosystem`, including the same-day DELETE on re-run); `beehiiv_posts` is a rolling **90-day** `_safe_replace` of confirmed sends with email (delivered/opens/unique opens/open rate/clicks/click rate/unsubscribes/spam reports) + web (views/clicks) stats. Auth is `BEEHIIV_API_KEY` — the **REST key, not the browser-OAuth MCP server** (see "MCP servers"); a scheduled job can't do OAuth. `refresh_beehiiv_publications` / `refresh_beehiiv_posts`, `--mode=beehiiv`, run in the `beehiiv` job of `refresh.yml`. Added 2026-09-01. |
...
`refresh_cache.py main()` accepts `--mode={all,direct,opensincera,beehiiv,…}` (the full list is `_VALID_MODES`). Default is `all` (full sweep). Each source has a corresponding `refresh_<source>` function callable individually for ad-hoc work. DV Attention is folded into the full sweep — no `--mode=dv_attention` flag because the agentmail poll is cheap (~3s + however long DV's CSV is to parse).
...
**The beehiiv MCP server and the beehiiv data source are separate paths, on
purpose.** The MCP server is interactive-only (browser OAuth, per-user), so
it can't be used from `refresh.yml` or any scheduled job. The daily pull
therefore goes through the **REST API** with `BEEHIIV_API_KEY` in secrets —
`beehiiv_client.py`, see the data-sources table. Use the MCP server for
ad-hoc exploration from a local session; use the REST client for anything
that has to run unattended. Don't try to route the sweep through MCP.
