...
| **DoubleVerify IVT** | `dv_ivt_client.py` (`pull_dv_ivt`) | `dv_ivt` | DV Pinnacle invalid-traffic classification rows (Valid Traffic / Fraud/SIVT / Fraud/GIVT) per line per day. Same email pipeline as DV Attention; subject filter: `Unified Analytics Report: IVT`. The CSV **does not include impression counts** — the three rate columns are tautological 0/1 bucket labels — so the dashboard surfaces a *day-prevalence proxy*: `(days with any Fraud row) / (total days seen) × 100`, NOT an impression-weighted IVT%. Surfaces as the "IVT" column on Direct + PMP tables. Color bands: green <15%, amber 15-30%, red ≥30% of days. Added 2026-05-24. |
