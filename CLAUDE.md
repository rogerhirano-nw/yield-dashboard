...
| **DoubleVerify IVT** | `dv_ivt_client.py` (`pull_dv_ivt`) | `dv_ivt` | DV Pinnacle invalid-traffic classification rows (Valid Traffic / Fraud/SIVT / Fraud/GIVT) per line per day. Same email pipeline as DV Attention; subject filter: `Unified Analytics Report: IVT`. The CSV **does not include impression counts** — the three rate columns are tautological 0/1 bucket labels — so the dashboard surfaces *day-prevalence proxies*: `(days with detected SIVT or GIVT) / (total days seen) × 100`, NOT impression-weighted IVT%. Surfaces as **separate "SIVT" and "GIVT" columns** on Direct + PMP tables per MRC standard distinction (SIVT = data center / bot fraud / hijacked devices / emulators / app + site fraud, hard to detect; GIVT = self-identifying bots / declared crawlers, standard detection). Color bands: green <15%, amber 15-30%, red ≥30% of days. Added 2026-05-24. |
