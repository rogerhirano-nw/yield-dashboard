...
- **Documented the full server-side (S2S / Prebid Server) bidder configuration
  ...
  The same data is also published as a spreadsheet
  (`Newsweek_SSP_Params_Assertive_Yield_Migration.xlsx`: Read me · Bidder roster
  · Account-level IDs · Parameters (tidy) · Desktop · Mobile), regenerated by
  `build_spreadsheet.py`. Docs-only; no code paths touched. Indexed in CLAUDE.md
