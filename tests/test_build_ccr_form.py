...
def test_build_facts_exclusions_and_kpis():
    lis = [
        _li(1, JEEP),
        _li(2, JEEP, status="DRAFT"),
        _li(3, JEEP + "-Apple-News"),                       # excluded
    ]
    f = ccr.build_facts([_order()], lis)
    assert [li["id"] for li in f.included] == ["1", "2"]
    assert [li["id"] for li, _ in f.excluded] == ["3"]
    assert any("LI 2" in w and "DRAFT" in w for w in f.warnings)
...
def test_build_facts_rejects_order_with_nothing_measurable():
    with pytest.raises(ValueError):
        ccr.build_facts([_order()], [_li(1, "x-Newsletter")])
...
def test_fill_template_writes_the_form(tmp_path):
    ...
    # No impression breakdown in the partner rows (Roger, 2026-09-23).
    assert all(c.value is None for row in md.iter_rows(min_row=9, max_row=18, max_col=6)
               for c in row)
    assert "Q4 2024 - $128k" not in wb.sheetnames
